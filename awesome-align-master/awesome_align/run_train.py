...
from awesome_align.train_utils import _sorted_checkpoints, _rotate_checkpoints, AdamW, \
                    ...
                    if gold_path is not None:
                        try:
                            ...
                            self.examples.append(
                                (ids_src, ids_tgt, bpe2word_map_src, bpe2word_map_tgt, gold_word_pairs))
                        except:
    ...
    def collate(examples):
        ...
        if args.n_gpu > 1 or args.local_rank != -1:
            ...
            guides = model.get_aligned_word(examples_src, examples_tgt, bpe2word_map_src, bpe2word_map_tgt, args.device,
                                            ...
                                            softmax_threshold=args.softmax_threshold, word_aligns=word_aligns)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.train_so or args.train_co:
                ...
                loss = model(inputs_src=inputs_src, inputs_tgt=inputs_tgt, attention_mask_src=attention_mask_src,
                             ...
                             extraction=args.extraction, softmax_threshold=args.softmax_threshold,
                             train_so=args.train_so, train_co=args.train_co)
            ...
            if args.train_tlm:
                ...
                for rand_id in rand_ids:
                    ...
                    for lang_id in [1, 2]:
                        inputs_srctgt, labels_srctgt = mask_tokens(select_srctgt, tokenizer, args, select_langid,
                                                                   lang_id)
        ...
        with torch.no_grad():
            ...
            if args.train_tlm:
                ...
                for select_id in select_ids:
                    for lang_id in [1, 2]:
                        inputs_srctgt, labels_srctgt = mask_tokens(batch[3 + select_id * 2], tokenizer, args,
...
def main():
    ...
    # Required parameters
    parser.add_argument(
        "--train_data_file", default=os.path.join(base_dir, "../data/model_data/train.txt"),
    ...
    # Training objectives
    parser.add_argument("--train_mlm", default=True, action="store_true")
    parser.add_argument("--train_tlm", default=False, action="store_true")
    parser.add_argument("--train_tlm_full", default=False, action="store_true")
    parser.add_argument("--train_so", default=True, action="store_true")
    parser.add_argument("--train_psi", default=False, action="store_true")
    parser.add_argument("--train_co", default=False, action="store_true")
    # Supervised settings
    parser.add_argument(
        "--train_gold_file", default=os.path.join(base_dir, "../data/model_data/train_gold.txt"), type=str,
        help="Gold alignment for training data"
    )
    parser.add_argument(
        "--eval_gold_file", default=os.path.join(base_dir, "../data/model_data/dev_gold.txt"), type=str,
    ...
    parser.add_argument(
        "--eval_data_file", default=os.path.join(base_dir, "../data/model_data/dev.txt"), type=str,
        help="An optional input evaluation data file to evaluate the perplexity on (a text file).",
    )
    parser.add_argument(
        "--should_continue", action="store_true", default=False,
    ...
    parser.add_argument(
        "--num_train_epochs", default=300.0, type=float, help="Total number of training epochs to perform."
    )
    parser.add_argument(
        "--max_steps",
        default=-1,
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        checkpoints = [os.path.join(args.output_dir, file_dir) for file_dir in os.listdir(args.output_dir) if
                       os.path.isdir(os.path.join(args.output_dir, file_dir))]
        ...
        for checkpoint in checkpoints:
            ...
            result = dict((k + "_{}_steps".format(global_step), v) for k, v in result.items())
            results.update(result)

            with open(os.path.join(args.output_dir, "all_eval_results.txt"), "a+") as writer:
